...
const updateClient = asyncHandler(async (req, res) => {
  const { shopName, clientName, phoneNumber, address, email, active } = req.body;
...
const deleteClient = asyncHandler(async (req, res) => {
  ...
  if (client) {
    // Check if client has receipts
    const receiptCount = await Receipt.countDocuments({ clientId: client._id });
    if (receiptCount > 0) {
      // Instead of deleting, mark as inactive
      client.active = false;
      await client.save();
      res.json({ message: 'Client has receipts. Marked as inactive instead of deleting.' });
    } else {
